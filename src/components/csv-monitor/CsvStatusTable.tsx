...
import knownHeaders from "@/../backend/src/pipeline/known_headers.json";
            ...
            data.map((entry) => {
              ...
              return (
                <TableRow key={entry.id} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => onRowClick(entry)}>
                  ...
                  <TableCell className="py-3 px-4 text-sm text-muted-foreground" title={entry.extracted_fields.join(", ")}>
                    {(() => {
        ...
        <DialogContent className="max-w-5xl">
          ...
          {moreDialogEntry && (
            <div className="space-y-4 overflow-x-auto">
              <div>
                <strong>Fields ({moreDialogEntry.extracted_fields.length}):</strong>
                <div className="flex flex-wrap gap-2 mt-2">
                  {moreDialogEntry.extracted_fields.map((field, idx) => {
                    let color: string | undefined = undefined;
                    let fontWeight: string | undefined = undefined;
                    if (CRITICAL_HEADERS.includes(field)) {
                      color = 'black';
